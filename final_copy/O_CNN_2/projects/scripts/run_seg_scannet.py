...
def test():
  ...
  ckpt = 'logs/scannet/mix3d_v1_scannet/checkpoints/00350.model.pth'
  if args.ckpt != '\'\'': ckpt = args.ckpt   # use args.ckpt if provided
  cmds = [
      'python segmentation.py',
      '--config configs/seg_scannet_eval.yaml',
      'SOLVER.gpu  {},'.format(args.gpu),
      'SOLVER.eval_epoch 1',  # voting with 72 predictions
  ...
  cmds = [
      ...
      '--path_pred logs/scannet/mix3d_v1_scannet_eval_test_{}'.format(args.alias),
      '--path_out logs/scannet/mix3d_v1_scannet_eval_test_seg_{}'.format(args.alias),
