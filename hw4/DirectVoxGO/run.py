...
def config_parser():
    ...
    # hw argument
    parser.add_argument("--test_json_path", type=Path)
    parser.add_argument("--test_out_dir", type=Path)
    parser.add_argument("--render_test_no_gt", action='store_true')
    return parser


@torch.no_grad()
# TODO: add filename argument -> save picture with filename
def render_viewpoints_no_gt(
        model, render_poses, HW, Ks, ndc, imgs_name, render_kwargs,
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            ...
            filename = os.path.join(savedir, f'{imgs_name[i]}.png')
...
def load_everything(args, cfg):
    ...
    kept_keys = {
            ...
            'poses', 'render_poses', 'images', 'imgs_name'}
    for k in list(data_dict.keys()):
        if k not in kept_keys:
            data_dict.pop(k)

    # construct data tensor
    if not args.render_test_no_gt:
...
if __name__=='__main__':
    ...
    # load model for rendering (new views)
    if args.render_test or args.render_test_no_gt or args.render_train or args.render_video:
        if args.ft_path:
        ...
        if args.render_test_no_gt:
            model = utils.load_model_no_gt(model_class, ckpt_path).to(device)
        else:
    ...
    if args.render_test_no_gt:
        print('All results are dumped into', args.test_out_dir)
        rgbs, depths, bgmaps = render_viewpoints_no_gt(
                render_poses=data_dict['poses'],
                # HW, Ks: list / array
                HW=data_dict['HW'],
                Ks=data_dict['Ks'],
                imgs_name=data_dict['imgs_name'],
                savedir=args.test_out_dir, dump_images=args.dump_images,
                **render_viewpoints_kwargs)
