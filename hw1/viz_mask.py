...
if __name__ == '__main__':
    ...
    img = imageio.imread(img_path) #(512, 512, 3), original pic
    seg = imageio.imread(seg_path) #(512, 512, 3), mask pic
    
    masks=read_masks(seg, img.shape) #(512, 512)
    
    cs = np.unique(masks)

    for c in cs:
        mask = np.zeros((img.shape[0], img.shape[1])) #(512, 512)
